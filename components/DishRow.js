  ...
  return (
    <>
      <TouchableOpacity onPress={() => setIsPressed(!isPressed)}
        //---> This is hidding the border bottom when buttons are showing     
        className={`bg-white border p-4 border-gray-200 flex-row ${isPressed && "border-b-0"}`}>
        <View className="flex-1">
          <Text className="text-lg mb-1">{name}</Text>
          <Text className="text-gray-500">{description}</Text>
          <Text className="mt-2 text-gray-400">{price} Usd</Text>
        </View>
        ...
        <View className="bg-white px-4">
          <View className="flex-row items-center space-x-2 pb-3">
            <TouchableOpacity disabled={!items.length} onPress={removeItemFromBasket}>
              <FontAwesomeIcon size={40} color={items.length > 0 ? "#00CCBB" : "gray"} icon={faMinusCircle} />
            </TouchableOpacity>
            <Text>{items.length}</Text>
            <TouchableOpacity onPress={addItemToBasket}>
              <FontAwesomeIcon size={40} color='#00CCBB' icon={faPlusCircle} />
            </TouchableOpacity>
          </View>
        </View>
      )}
